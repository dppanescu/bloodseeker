...
def seed_detail(seed_id: int, request: Request):
    ...
    try:
        ...
        try:  risk_min = int(qp.get("risk_min") or "60")
        except Exception: risk_min = 60
        try:  risk_max = int(qp.get("risk_max") or "100")
        except Exception: risk_max = 100
        risk_min = max(0, min(100, risk_min)); risk_max = max(0, min(100, risk_max))
        if risk_min > risk_max: risk_min, risk_max = risk_max, risk_min

        risk_count = db.query(func.count(Variant.id)).filter(
            Variant.seed_id == seed_id,
            Variant.risk_score.isnot(None),
            Variant.risk_score >= risk_min,
            Variant.risk_score <= risk_max,
        ).scalar() or 0

        # CT hits agregat + top domenii cu hit (best-effort)
        ct_hits_total, ct_domains = 0, []
        try:
            ct_hits_total = db.query(func.coalesce(func.sum(CheckRun.ct_hits), 0)) \
                .join(Variant, CheckRun.variant_id == Variant.id) \
                .filter(Variant.seed_id == seed_id).scalar() or 0
            ct_domains = db.query(Variant.domain, func.coalesce(func.sum(CheckRun.ct_hits), 0).label("hits")) \
                .join(CheckRun, CheckRun.variant_id == Variant.id) \
                .filter(Variant.seed_id == seed_id) \
                .group_by(Variant.domain) \
                .order_by(func.sum(CheckRun.ct_hits).desc()) \
                .limit(50).all()
        except Exception:
            ct_hits_total, ct_domains = 0, []
        ...
        if sort == "-risk":
            ...
            order_cols = [Variant.id.desc()]
        elif sort == "id":
            order_cols = [Variant.id.asc()]
        elif sort == "domain":
        ...
        generator_opt = ""
        try:
            generator_opt = (seed.options or {}).get("generator", "")
        except Exception:
            generator_opt = ""
        if generator_opt == "both":
            generator_lines = ["Simplu", "DNSTwist"]
        elif generator_opt == "dnstwist":
            generator_lines = ["DNSTwist"]
        else:
            generator_lines = ["Simplu"]

        # TLD-uri pentru pop-up
        try:
            tlds = (seed.options or {}).get("tlds", []) or []
        except Exception:
            tlds = []
...
async def add_seed(request: Request):
    ...
    opts: dict = {}
    # păstrează tipul de generator în options, ca să-l afișăm în UI
    if generator in {"simple", "dnstwist", "both"}:
