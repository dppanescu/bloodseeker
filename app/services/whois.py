...
def upsert_cache(db: Session, variant, norm: Mapping[str, Any], source: str = "whoisxmlapi") -> None:
    ...
    db.execute(
        ...
        VALUES (:vid, :dom, CAST(:data AS JSONB), :src, NOW())
        ...
        {
            "vid": variant.id,
            "dom": variant.domain.lower(),
            "data": json.dumps(norm if isinstance(norm, dict) else {}),
            "src": source,
        },
    )
    db.commit()
    ...
    db.execute(
        stmt,
        {
            "vid": variant.id,
            "dom": variant.domain.lower(),
            "data": dict(norm),   # dict nativ; driverul îl serializează corect în JSONB
            "src": source,
        },
    )
    db.commit()
